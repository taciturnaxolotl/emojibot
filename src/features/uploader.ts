	...
	app.anyMessage(async ({ payload, context }) => {
		...
		if (res.ok && aliases.length > 0) {
			for (const alias of aliases) {
				...
				aliasForm.append("token", process.env.SLACK_BOT_USER_TOKEN ?? "");
				...
				try {
					...
					if (aliasRes.ok) {
						...
						console.log(`💾 Added alias ${alias} for ${primaryName}`);
					} else {
						failedAliases.push(alias);
						console.log(`⚠️ Failed to add alias ${alias}: ${aliasRes.error}`);
					}
				} catch (err) {
					failedAliases.push(alias);
